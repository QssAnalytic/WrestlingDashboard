        ...
        {filterDialog?.[id] && sortedData ? (
          <div className="dropdown-options  absolute top-12 right-0 w-full">
            <ul className="z-10 rounded border border-[#373A45] bg-[#0F1322] my-1 text-[#AAADB6] overflow-y-auto h-56 text-sm">
              <div className='w-full flex items-center justify-end bg text-center'>
                <input type="text" placeholder={`Search ${name}...`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="capitalize border border-transparent bg-transparent text-[#AAADB6] focus:outline-none px-2 py-2 rounded " />
              </div>
